...
const $mensageFinJuego = document.querySelector('#fin-juego');
...
function evaluarFinJuego() {
    if (document.querySelectorAll('.cuadro').length === 0) {
        ...
        $mensageFinJuego.querySelector('strong').textContent = turnos.toString();
        $mensageFinJuego.style.display = 'block';
    }
}
configurarJuego();
